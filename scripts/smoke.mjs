...
import { reduce, createGame, toPublicState, houseDefault } from '../src/engine/index.ts';
...
async function run(state, label, checks) {
  ...
  await page.route('http://fake.supabase.local/**', async (route) => {
    ...
    if (url.includes('/rest/v1/games')) return json({ join_code: 'ABC123', status: state.phase === 'lobby' ? 'lobby' : 'playing', ruleset: rs, public_state: toPublicState(state), version: state.version });
...
const h = await (async () => {
  ...
  const text = await page.innerText('body');
  for (const c of ['New game', 'House default', 'Classic 2s wild', 'Rounds (7)', 'three runs of 4']) if (!text.toLowerCase().includes(c.toLowerCase())) errors.push(`[setup] missing ${c}`);
